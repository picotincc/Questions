...
- 服务器发回一个HTML响应
- 浏览器开始渲染HTML，并请求外部资源。
- 渲染过程：解析html以构建dom树 -> 构建render树 -> 布局render树 -> 绘制render树
- HTML/SVG/XHTML，解析这三种文件会产生一个DOM树
- 再解析CSS，解析CSS会产生CSS规则树。之后与DOM树一起，构建Render树。（Render树只有Dom树的可见元素）
- JavaScript脚本，主要是通过 DOM API 和 CSSOM API 来操作 DOM Tree 和 CSS Rule Tree.
- js的下载和执行会阻塞Dom树的构建，脚本标记为defer之后可以滞后执行。
- 样式表必须在脚本之前加载和解析，因为脚本可能会在解析的过程中请求样式信息。
